...
この名前ベクトルを取得したら、**stringr** パッケージの `str_extract()` で以下の正規表現を使用することにより、名前から日付を抽出できます。ファイル名に含まれているすべての数字（ダッシュやスラッシュなどの中央にある他の文字を含む）を抽出します。 **stringr** の詳細については、[文字列・文字列型データ](#characters-strings) の章をご覧ください。

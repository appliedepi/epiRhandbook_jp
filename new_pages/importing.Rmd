...
Sometimes, you may want to avoid importing a row of data. You can do this with the argument `skip =` if using `import()` from **rio** on a .xlsx or .csv file. Provide the number of rows you want to skip.一部の行のインポートを避けたい場合は以下のようにします。 .xlsx または .csv ファイルで **rio** から `import()` を使用する場合は、引数 `skip =` を使用してこれを行うことができます。 スキップする行数を指定します。

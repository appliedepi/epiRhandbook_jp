...
まず、シミュレートされたエボラ出血熱の流行から、整理された症例のラインリストをインポートします。[クリックして "整理された" ラインリストをダウンロード](https://github.com/appliedepi/epirhandbook_eng/raw/master/data/case_linelists/linelist_cleaned.rds)します（.rds ファイルとして）。Import data with the `import()` function from the **rio** package (it handles many file types like .xlsx, .csv, .rds - see the \[Import and export\] page for details).
...
[***CAUTION:*** Typically when cleaning one should create a new column (e.g. `hosp_name_clean`), but for ease of demonstration we show modification of the old column]{style="color: orange;"}
...
[***CAUTION:*** Joins are case-specific! Therefore it is useful to convert all values to lowercase or uppercase prior to joining. See the page on characters/strings.]{style="color: orange;"}

...
`getMatches()` で重複の可能性があるものを確認することができます。データフレームを `dfA =` と `dfB =` の両方で提供し、`fastLink()` 関数のアウトプットを `fl.out =` とします。`fl.out` must be of class `fastLink.dedupe`, or in other words, the result of `fastLink()`.`fl.out` は `fastLink.dedupe` クラスでなければならず、言い換えれば`fastLink()` の結果である必要があります。
...
一番右の列は重複するIDを表しており、最後の2行は2行目と3行目と重複している可能性が高いことが分かります。
...
Now we can bind the data frame columns together, with the correct row order. これで、正しい行順序でデータフレーム列を結合できます。 Note that some columns are duplicated and will require cleaning with `rename()`. 一部の列が重複しており、rename（）でクリーニングする必要があることに注意してください。Read more aboout `bind_rows()` [here](https://dplyr.tidyverse.org/reference/bind.html).bind_rows()について詳しくはこちらをご覧ください。

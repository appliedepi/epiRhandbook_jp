...
-   `data =` 引数は、`linelist` データフレームを設定します。

-   `age_group =` 引数 (Y 軸) には、カテゴリカルな年齢の列名 (引用符で囲む) を設定します。

-   `split_by =` 引数（x 軸）には、性別の列を設定します。
...
`split_by =` または `age_group =` の列に欠損値 `NA` がある行は、`NA` としてコード化されている場合、上記のファセットは実行されません。初期値では、これらの行は表示されません。しかし、`na.rm = FALSE` を指定することで、隣接する棒グラフや上部に別の年齢層として表示させることができます。
...
デフォルトでは、棒グラフはカウント（%ではない）、各グループの破線の中線が表示され、色は緑と紫になります。これらのパラメータはそれぞれ、以下のように調整することができる。

また、美的テーマやラベルの調整など、標準の ggplot() "+" シンタックスを使用して、追加の ggplot() コマンドをプロットに追加することができます。

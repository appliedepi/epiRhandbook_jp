...
本章では、**janitor**、**dplyr**、**gtsummary**、**rstatix**、R の**base**（以下、ベース R）を使って、データを要約したり、記述統計表を作成したりする方法を紹介します。
...
-   **gtsummary** から `tbl_summary()` を使用して、出版原稿レベルの詳細な表を作成します。
...
linelist %>%                                  # 症例ラインリスト
  ...
  adorn_ns(position = "front") %>%            # 「カウント数 (%)」となるように表示を変更
  adorn_title(                                # タイトルを調整
...
[文字列型データ() の章では、 `unite()`や `paste0()`など、列を結合するためのさまざまなオプションについて説明しています。この使用例では、 `unite()` よりも柔軟性があり、`paste0()`よりもシンプルな構文である `str_glue()` の利用を提唱します。
...
linelist %>% 
  ...
  summarise(
    ...
    n_death  = sum(outcome == "Death", na.rm=T),    # グループ内で転帰が「死亡」の行数
    n_recover = sum(outcome == "Recover", na.rm=T), # グループ内で転帰が「回復」の行数
  ) %>% 

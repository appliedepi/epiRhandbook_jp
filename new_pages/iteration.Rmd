...
以下のコードチャンクを実行すると、分析に必要なパッケージが読み込まれます。このハンドブックでは **pacman** の `p_load()` を主に使用しています。`p_load()` は、必要に応じてパッケージをインストールし、[そして]{.ul}パッケージの読み込みまで行ってくれます。また、既にインストール済みのパッケージは、R の **base** パッケージの `library()` を使用して読み込むこともできます。パッケージの詳細については ［R の基礎 {\# basics}］の章の 3.7 パッケージ を参照してください。

...
-   入力された `.x`（病院名）は `.f` [の中において]{.ul}、`linelist_split` リスト内のインデックスとなります。反復処理が繰り返すたびに一つのデータフレームが `export()` に渡されます。
...
-   `export()` で指定するファイルパス内には、`here()` を使用した後に `/` と `.x` を記載し、ファイルが各病院名がファイル名になるようにします
...
# 6つの病院の 「名前」のベクトルに対してマッピングする（既に作成済）
# ggplot 関数を用いる
...
その他に頻繁に用いられる例としては、列全体に対して関数をマッピングする場合、が挙げられます。以下では、`linelist` データフレーム内の数値列に対して `t.test()` 関数を `map()` し、性別で数値を比較していきます。
...
-   `t.test()` のカッコ内は、
    ...
    -   `.x` は反復処理の繰り返しの際に `t.test()` 関数に適用される列を表します
    -   2つ目のチルダ（`~`）は、上で説明した通り、t 検定の数式の一部です
    -   `t.test()` 関数は、チルダ（`~`）の右側にバイナリ変数の列をあてます。ここでは `linelist$gender` というベクトルをあてています（`select()` に gender を含まないように注意してください）
...
リスト内の要素の名前を抽出するには、R の基本パッケージ **base** の `names()` を使うと簡単にできます。今回は、`names()` を `t.test_results` に適用することでそれぞれのサブリスト名（t 検定に用いられた変数の名前）を抽出します。
...
さらに、より簡単で柔軟性の高い方法として、**purrr** パッケージの関数 `map()` と `pluck()` を使用し、同様の結果を抽出する方法を以下で説明していきます。
...
また、リスト内の[すべての]{.ul}最上階層から、より下の階層にある要素を抽出するには、`map()` を用いて `pluck()` をそれぞれの最上階層の要素に適用させるとできます。例えば、以下のコードでは `t.test_results` リストに含まれるすべてのサブリストから "p.value" という要素を抽出しています。この場合、`t.test_results` は反復処理対象となる `.x` であり、`pluck()` は反復される関数である `.f` であり、"p-value" はその関数に対して適用されています。
...
この場合、`estimate` という要素を抽出したいのですが、問題はその中に [2 つの]{.ul}要素 （`mean in group f` と `mean in group m`）が含まれていることです。そのため、`map_chr()` や `map_dbl()` を用いてベクターに変換することはできません。その代わり、`tibble()` 内で `map()` を用いることで [tibble 内にリスト要素の列]{.ul}を作成することができます！
...
**purrr** パッケージの関数を用いる際にはリストを扱うことが多いため、リストを変換するのに用いられる **purrr** 関数をいくつか簡単に説明します。**purrr** パッケージに含まれる関数のより詳しい内容を知りたい方は、本章の最後に記載されている参考資料をご覧ください。

-   `list_modify()` にはいくつか用途がありますが、例えばリストの要素を取り除く、といった場合に使用されます\
-   `keep()` は、引数 `.p =` で指定された要素、もしくは `.p =` に与えられた関数が TRUE となる要素をキープします\
-   `discard()` は、引数 `.p =` で指定された要素、もしくは `.p =` に与えられた関数が TRUE となる要素を取り除きます\
-   `compact()` は、全ての空の要素を取り除きます

それでは、`map()` を使用して複数のファイルをインポートし結合した [16.3 purrrr and lists - map()](#iter_combined) で作成したリストである `combined`（6 つのデータフレームを含むリスト）を用いて例を示します。
...
また、何かの基準を指定して要素を取り除くこともできます。その場合は、「述語」関数（TRUE か FALSE を判定する関数）を `.p =` に渡します。チルダ（`~`）を関数の前に用い、リスト要素を表す `.x` を用います。`keep()` を使用し、TRUE と判定されたリスト要素をキープします。逆に、TRUE と判定されたリスト要素を取り除きたい場合は、`discard()` を使用してください。以下に例を示します。
...
また、述語関数にそれぞれのリスト内の要素や列を用いることもできます。例えば、以下では、`ct_blood` 列の平均値が 25 よりも大きいリスト要素を取り除いています。
...
**purrr** の代わりに、R の **base** パッケージに含まれている種々の "apply" 関数を使用して反復処理を行うこともできます。詳しくは、[こちら](https://www.datacamp.com/community/tutorials/r-tutorial-apply-family) をご覧ください。

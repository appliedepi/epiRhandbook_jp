...
例として、[for ループ]{.ul}の中に、シーケンスの値ごとに実行されるオペレーションを加えます。この例では、シーケンスの値（列名）を使って、`linelist` の列名を一つずつ[指定して]{.ul}（サブセットして）います。オペレーションコードでは、[R の基礎](#basics) の章で説明したように、二重ブラケット `[[ ]]` を使用してサブセットします。そして、ループにより指定された列は `is.na()`、次に `sum()` に渡され、列内の欠損値の数を算出します。結果として、コンソールに欠損値の数が列ごとに表示されます。
...
シーケンスに添字を使うと、ループの出力結果を格納する[コンテナ]{.ul}のインデックスとして、`i` 番号を簡単に使用できるという利点があります。詳しくは、次のオペレーションのセクションで例を示して解説します。

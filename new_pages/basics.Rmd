...
このページは R の要点をおさらいします。これは包括的なチュートリアルを意図するものではなく、あなたの記憶をリフレッシュするのに有効なページを提供します。[Resource for learning](#learning) のセクションにより包括的なチュートリアルのリンクがあります。

このページの一部は、[R4Epis project](https://r4epis.netlify.app/) の許可を受けて掲載されています。

STATA、SAS、Excel から R へ移行するときのヒントは[Transition to R](#transition_to_r)のページをご覧ください。
 

```{r, echo=F}
# 整理されたエボラ出血熱のラインリストをインポートする
...
[R project のウェブサイト](https://www.r-project.org/about.html)に記載されている通り、R はプログラミング言語であり、統計的機械計算と可視化のための環境でもあります。非常に汎用性が高く、拡張性があり、コミュニティ主導型です。

**費用**

R の使用量は無料です！フリーとオープンソースについて脂質のある人のコミュニティには強固な倫理的価値観があります。

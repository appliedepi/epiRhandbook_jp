...
```{r}
followups %>% 
  count(contact_id, date_of_followup) %>%   # 接触者ID毎に、フォローアップ日の頻度を算出
  filter(n > 1)                             # 1日2回以上フォローアップが行われた日を表示
```

ここで扱っているサンプルデータでは、IDのない記録だけが該当しています。下のコードで、重複した記録を排除していきます。重複記録を排除することで、1 日に 1 人につき 1 つのフォローアップ記録のみ残るようになります。 詳細は、[重複データの排除] (\#deduplication) の章をご覧ください。ここでは、最新のフォローアップ記録が正しいものであると仮定します。また、下のコードでは、一緒に `followup_number` 列（フォローアップの「日」ごとに作成され、1 ～ 14 日まである。）をクリーニングします。

```{r, warning=F, message=F}
followups_clean <- followups %>%
  
  # 重複記録の排除
  group_by(contact_id, date_of_followup) %>%        #接触者ID、フォローアップ日ごとに記録をグループ化する
  arrange(contact_id, desc(date_of_followup)) %>%   # 接触者ID毎に、フォローアップ日の降順で記録を並び替える（日付の新しい順）
  slice_head() %>%                                  # 接触者ID毎に、最新のフォローアップ日の記録のみ残す  
  ungroup() %>% 
  
  # 他の前処理
  mutate(followup_number = replace(followup_number, followup_number > 14, NA)) %>% # フォローアップ日のエラーの処理
  drop_na(contact_id)                               # 接触者IDが欠損してる記録を排除する
```

それぞれのフォローアップについて、フォローアップの結果（接触者と連絡が取れたか、また、連絡が取れた場合は接触者に症状があったか、等）が記録されています。**janitor** パッケージの `tabyl()` 関数、または **base** R の `table()` 関数に `followup_status` 変数を指定して実行すると、フォローアップの結果の分布が確認できます。（詳細は、[記述統計表の作り方] (\#tables-descriptive) の章をご覧ください。）
...
### フォローアップ状況を時系列でプロットする {.unnumbered}

日付データは連続しているので、`date_of_followup` を x 軸に指定したヒストグラムを使用し、接触者追跡記録のデータを時系列でプロットできます。`aes()` 関数内の `fill =` 引数に `followup_status` 変数を指定することで、「積み上げ型」のヒストグラムになります。`labs()` 関数内の `fill =` 引数を使用して、凡例のタイトルを作成することができます。

このサンプルデータセットを使用して作成したヒストグラムでは、接触者の分布が波状に確認され（おそらく感染者の流行の波と連動している）、フォローアップが行われていない場合が多く、流行の過程でフォローアップの達成状況が改善されていないことがわかります。
...
[[**注意点**]{.ul}***:*** 同一コードで複数のプロットを作成する場合（複数の管轄区域に同じプロットを提示する場合など）、データの完成度やデータの構成が異なっていても、凡例は同じように表示されるようにする必要があります。例えば、フォローアップの結果のすべてのカテゴリーがデータに含まれていない図があるかもしれませんが、含まれていない結果のカテゴリーについても凡例に表示したい場合です。上のような ggplot を使用した図では、`scale_fill_discrete()` 関数内で `drop = FALSE` に指定すると、図に表示されていないカテゴリーを含むすべてのカテゴリーを凡例に表示することができます。表の場合は、すべての因子（ファクター）レベルのカウントを表示する `tabyl()` 関数を使用するか、または、**dplyr** パッケージの `count()` 関数を使用する場合は、`.drop = FALSE` を追加し、すべての因子レベルのカウントを含めることができます。]{style="color: orange;"}

### フォローアップ状況を接触者ごとにプロットする {.unnumbered}

調査対象のアウトブレイクの規模が小さい場合は、各接触者のフォローアップ状況を個別に確認したい場合があります。この接触者追跡記録のデータ（`followups` データセット）には、フォローアップの日にち「番号」を示す列がすでに含まれています（1 ～ 14 日まで一日ごとに列が作成してあります)。フォローアップ日ごとの列がデータにない場合は、曝露日（調査対象の接触者が感染者と接触した日）とその接触者に対してフォローアップを開始した日の差を計算し、1 ～ 14 日まで一日ごとに列を作成する必要があります。
...
### フォローアップ状況をクループ別にプロットする {.unnumbered}

このような接触者追跡に関するデータは、疫学調査上の意思決定のために日ごとまたは週ごとに閲覧されていることが多いと思います。`group_by()` 関数で指定する列を調整することにより、地域別や調査チーム別など、より意味のある集計結果をプロットすることができます。
...
接触者調査の成果を評価するために、進捗や目標達成状況を様々なレベルで細分化された期間で算出し、モニターすることができる重要業績評価指標（Key Performance Indicator: KPI）があります。KPIには多くの種類がありますが、計算方法と基本的な表のフォーマットを理解すると、様々な異なるKPIを入れたり外したりすることは非常に簡単です。

接触者調査のKPIについては、[ResolveToSaveLives.org](https://contacttracingplaybook.resolvetosavelives.org/checklists/metrics) など、参考となる資料がたくさんあります。KPI作成の大部分は、データ構造を確認し、すべての選択/除外基準を考えることです。以下に、Go.Data のメタデータ構造を使用した例を示します。

+-----------------------------------------+------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------------------------------------------+-----------------------+
| 視点                                    | KPI                                                                          | Go.Data 分子                                                                                                                               | Go.Data 分母          |
+=========================================+==============================================================================+============================================================================================================================================+=======================+
| プロセスの評価 - 接触者追跡調査の迅速さ | 全感染者のうち、感染報告から24時間以内に疫学調査が行われ、隔離された者の割合 | COUNT OF `case_id` WHERE (`date_of_reporting` - `date_of_data_entry`) \< 1 day AND (`isolation_startdate` - `date_of_data_entry`) \< 1 day | COUNT OF `case_id`    |
+-----------------------------------------+------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------------------------------------------+-----------------------+
| プロセスの評価 - 接触者追跡調査の迅速さ | 全接触者のうち、接触報告から24時間以内に通知され、隔離された者の割合         | COUNT OF `contact_id` WHERE `followup_status` == "SEEN_NOT_OK" OR "SEEN_OK" AND `date_of_followup` - `date_of_reporting` \< 1 day          | COUNT OF `contact_id` |
+-----------------------------------------+------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------------------------------------------+-----------------------+
...
このような表の作成手順は、接触者の追跡記録データの構造によりって変わります。 **dplyr** パッケージを使用してデータを要約する方法についての詳細は、[記述統計表の作り方] (\#tables-descriptive) の章をご覧ください。

ここでは、データの変化に合わせて連動する表を作成します。結果を面白くするために、特定の日に `report_date` （この表を提示し、疫学調査の進捗を報告する日）を設定し、表を作成します（この例では、2020年6月10日を選びます）。データは `report_date` でフィルタリングされ、`report_date` の日、または、`report_date` より以前に報告された記録のみ残ります。
...
1)  接触者追跡記録のデータ（`followups` データセット）を選択し、各接触者について、以下 3 つの関心のある指標を算出します。\

-   一番最近行ったフォローアップの日付（フォローアップの状況は問わない）\
-   接触者と連絡が取れたフォローアップのうち、最も新しい日付\
-   接触者と連絡が取れた最新のフォローアップでの、接触者の状況（例：症状あり又は症状なし）\

2)  ステップ 1 で作成したデータを、曝露日（感染者と接触した日）などの接触者に関する他の情報を含む接触者のデータ（`contacts` データセット）に結合します。次に、各接触者について、曝露日からの日数など、関心のある指標を算出します。\
3)  結合したデータを、管轄区域別（`admin_2_name`）にグループ化し、管轄区域別の要約統計量を算出します。\
4)  最後に、作成した表の形式を整えます。

では、以上の手順を実際に R で実行していきましょう。

ステップ 1：接触者追跡記録のデータ（`followups` データセット）を整備し、必要な指標を算出する。
...
ステップ 2：次に、作成したデータセットを接触者のデータ（`contacts` データセット）に結合し、他に必要な指標を算出します。
...
2 つのデータセットを結合すると、データは以下のようになります。接触者のデータ（`contacts` データセット）に含まれていた列は右側に、算出した関心のある指標は一番右に表示されます。
...
[ヒートマップ] (\#heatmaps) の章で解説したように、`geom_tile()` 関数を使用し、「誰が誰から感染したか」を可視化するヒートマップを作成することができます。

Go.Data では、接触者のデータ（`contacts` データセット）に新しい接触者が追加されると、API によって、感染者と接触者の関連データ（`relationships` データセット）に、その接触者と接触した感染者との関係情報が追加されます。`relationships` データセットには、各接触者と接触した感染者の情報が含まれているので、このデータセットを使用すると比較的簡単にヒートマップを作成することができます。以下に、`relationships` データセットの最初の 50 行を表示しています。
...
最後に、接触者がどの年代の感染者から感染したかを表すヒートマップを作成します。

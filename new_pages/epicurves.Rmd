...
より一般的なデータに対して移動平均を算出する場合はこのハンドブックの [移動平均](#移動平均-1) を参照して下さい。
...
-   各グループの色分けをマニュアルで設定するには、 `scale_fill_manual()` を使用します (注意: `scale_color_manual()` と区別します!)。

    -   色のベクトルを適用するには、`values =` 引数を使用します。\

    -   欠損値の色を指定するには、`na.value =` を使用します。\

    -   `labels =` 引数を使用して、凡例の項目名を変更します。念の為に、`c("old" = "new", "old" = "new")` のような名前付きベクトルとして提供するか、データ自体の値を調節してください。

    -   凡例のタイトルを指定するためには、`name =` を使用します。

-   色スケールやパレットに関する詳しい情報は、\[ggplot の基礎\]のページを参照してください。
...
グループ化された棒グラフを積み重ねる順番は、グループ化された列を Factor 型に分類することで調整出来ます。Factor型 のレベルの順番（とその表示ラベル）を指定することで調整します。詳しくは、\[因子（ファクタ）型データ\]または\[ggplot の基礎\]のページを参照してください。

グラフを作成する前に、**forcats** パッケージの `fct_relevel()` 関数を使って、\[因子（ファクタ）型データ\] のページで説明されているように、グループ化列をfactor 型に変換し、レベル順をマニュアルで調整してください。
...
以下の例では、`hospital` 列を上記のように統合した上で凡例の順番を `guides()` 関数を用いて逆にした上で "**Missing**" が凡例の一番下に来るように変更しています。
...
[***TIP:*** 凡例の順番を逆にするには、以下の **ggplot2** コマンドを使用します: `guides(fill = guide_legend(reverse = TRUE))`.]{style="color: darkgreen;"}

### 凡例の調整 {.unnumbered}

凡例やスケールについては、\[ggplot \]のページで詳しく説明しています。以下はそのハイライトです：

-   凡例のタイトルを編集するには、scale 関数か、`labs(fill = "Legend title")` を使用します(`color =` aesthetic を使用している場合は `labs(color = "")` を使用)\
-   `theme(legend.title = element_blank())` で凡例のタイトルを削除
-   `theme(legend.position = "top")` （"bottom", "left", "right" で凡例をそれぞれ下、左、右に配置。または"none" で凡例が削除されます）
-   `theme(legend.direction = "horizontal")` で凡例を水平に配置
-   `guides(fill = guide_legend(reverse = TRUE))` で凡例の順序を逆にする

### 棒グラフの横並び表示 {.unnumbered}

グルーブ化された棒グラフを横並びに表示する(積み重ねではなく)には、`geom_histogram()`{style="padding: 0px 1px; border-width: 1px; border-style: solid; border-color: rgb(51, 80, 116) !important; border-image: initial; background-color: rgb(26, 58, 99) !important; font-family: Osaka-Mono, monospace !important; font-size: 9pt !important;"} の中で、`aes()`{style="padding: 0px 1px; border-width: 1px; border-style: solid; border-color: rgb(51, 80, 116) !important; border-image: initial; background-color: rgb(26, 58, 99) !important; font-family: Osaka-Mono, monospace !important; font-size: 9pt !important;"} の外に置か`position = "dodge"`{style="padding: 0px 1px; border-width: 1px; border-style: solid; border-color: rgb(51, 80, 116) !important; border-image: initial; background-color: rgb(26, 58, 99) !important; font-family: Osaka-Mono, monospace !important; font-size: 9pt !important;"} で指定します。

2つ以上のグループがある場合、読みにくくなることがあります。代わりにファセット化された可視化を行った方が良いかも知れません。以下の例では、見やすくするために性別の欠測値を削除しています。
...
一般的な方法は、`xlim = c(min, max)` と `ylim = c(min, max)` を包含する `coord_cartesian()` コマンドを使用することです（それぞれの min と max の値を指定します）。これは、実際にデータを削除することなく「ズーム」として機能し、統計量や要約尺度において重要です。

また、scale_x\_date() 内で limits = c() を使用して、最初の日付と最後の日付を設定することもできます。
...
同様に、新しい症例が報告されていない場合でも、x軸を特定の日付（例えば現在の日付）まで伸ばしたい場合は、次のように指定します：

```{r eval=F}
scale_x_date(limits = c(NA, Sys.Date()) # 最後の日付を現在の日付に指定
```

[注意：y軸の区切りや範囲の設定には注意が必要です（例：0から30までを5ごとに分割: `seq(0, 30, 5)`). このようなやり方では、データが設定範囲外の値だった場合にグラフが削除される場合があります！]{style="color: red;"}

### 日付軸のラベルと罫線 {.unnumbered}

[***TIP：日付***軸のラベルはデータの集約とは関係ありませんが、視覚的にビン分割、日付ラベル、垂直罫線を揃えるために重要です]{style="color: darkgreen;"}
...
-   **ヒストグラムのビン分割が日、月、月曜始まりの週、月、年である場合：**
    ...
    -   日付のラベルの書式を指定するには `date_labels =` を使用します - ヒントは\[日付型データ\]のページを参照してください (改行には `\n` を使用)

-   **ヒストグラムのビン分割が日曜始まりの週：**

    -   `breaks =` と `minor_breaks =` を使用してそれぞれ一連の日付の区切りを指定します
    -   上記のように `date_labels =` と `expand =` を使用してフォーマットすることは可能です

注意点：

-   `seq.Date()` を使用して日付のベクトルを作成する方法については、\[ggplot の基礎\]のセクションを参照して下さい

-   日付ラベルを作成するためのヒントについては、このページまたは \[日付型データ\] ページを参照してください。

#### デモンストレーション {.unnumbered}

以下は、ビン分割とラベル、罫線が整列しているグラフと整列していないグラフのデモです：
...
ggplot(central_data) + 
  geom_histogram(
    ...
    breaks = weekly_breaks_central,    # このページで定義済み
...
ラインリストではなく、施設や地域などの集計データのみがあることが多くあります。`ggplot()` で流行曲線を作成することができますが、コードが少し異なります。このセクションでは、先ほどのデータ準備のセクションでインポートした `count_data` データセットを使用します。このデータセットは、`linelist` から日別の症例数をカウントしたデータが含まれています。最初の50行を以下に表示します。
...
日別カウントデータから流行曲線を作成することが出来ます。以下は以前のコードとの相違点です：

-   `aes()` 内で、日別カウントデータを `y =` に指定します (この場合、列名は `n_cases` です）。

-   `geom_histogram()` に引数 `stat = "identity"` を追加し、棒グラフの高さをデフォルトの行数ではなく、`y =` として指定します。\

-   棒グラフの間に縦の白線が入らないように、`width =` の引数を追加します。日別データの場合、1に設定します。週別カウントデータの場合は7に設定します。月別のカウントデータでは、白線が問題になります (各月で日数が異なるためです) - x 軸を因子型 (月) に変換して `geom_col()` を使用することを検討して下さい。
...
もしデータが週ごとの症例数であれば、以下と同じようになっているはずです（count_data_weekly という変数名にしています）：
...
`count_data_weekly` の最初の50行を以下に表示します。週単位で症例が集計されています。各週は、週の最初の日(デフォルトでは月曜日)で表示されます。
...
`x =` epiweek となるようにプロットします。`y =` counts を追加し、上で説明したように `stat = "identity"` を追加することを忘れないで下さい。
...
ggplot(data = count_data_weekly)+
  ...
  geom_histogram(
    mapping = aes(
      x = epiweek,           # x軸を疫学週に指定（日付型として）
      y = n_cases_weekly,    # y軸を週別カウントデータに指定
...
詳細な説明といくつかのオプションについては、\[移動平均\]のページを参照して下さい。以下は、**slider** パッケージで移動平均を計算するためのオプションの1つです。この方法では、移動平均はプロットする前にデータセット上で計算されます。

1)  データを必要に応じて集計します（日別、週別など）（\[データのグループ化\]ページを参照）。\
2)  **slider** パッケージの `slide_index()` で移動平均の列を新規に作成します。\
...
他の ggplot と同様に、ファセット化されたグラフ（「複数の小グラフを並べたグラフ」）を作成することができます。 \[ggplotの基礎\] ページで説明されているように、 `facet_wrap()` と `facet_grid()` のどちらかを使用することができます。ここでは `facet_wrap()` を使って説明します。流行曲線の作成においては1つの列に対してのみファセットする必要があるため、一般的に `facet_wrap()` の方が簡単です。

一般的な構文は `facet_wrap(rows ~ cols)` で、チルダ (\~) の左側はファセット化プロットの「行」に対する列の名前、チルダの右側はファセット化プロットの「列」に対する列の名前です。最も簡単な例では、チルダの右側に1つの列を指定します: `facet_wrap(~age_cat)`

**フリースケール**\
各ファセットの軸のスケールを同じ値に「固定」（デフォルト）するか、「自由」（ファセット内のデータに基づいて変更する）にするかを指定します。これは、`facet_wrap()` 内の `scales =` 引数で、"free_x" か "free_y" か "free" を指定することで出来ます。

**ファセットの列数と行数**\
`facet_wrap()` 内で `ncol =` または `nrow =` で指定することが出来ます。

**パネルの順番**\
順序を変更するには、ファセットを作成するために使用される因子型列のレベルの順序を変更します。

**外観**\
フォントサイズ、ヘッダ、色等は、以下のような引数を持つ `theme()` で変更することが出来ます：

-   `strip.text = element_text()` (サイズ、ヘッダ、色、角度等)
-   `strip.background = element_rect()` (例：element_rect(fill="grey"))
-   `strip.position =` (ファセットのタイトルが記載されるボックスの場所を指定："bottom", "top", "left", or "right")

**ファセットラベル**\
ファセットプロットのラベルは、列の「ラベル」を用いるか「labeller」の使用によって変更することが出来ます。

**ggplot2** の関数 `as_labeller()` を用いて、ラベラーを作成した後に、以下のように`facet_wrap()` の `labeller =` 引数にラベラーを指定します。
...
labellers に関してはこちらのリンク（[link](https://ggplot2.tidyverse.org/reference/labellers.html)）を参照して下さい

#### ファセットの背景にトータルの流行曲線を表示 {.unnumbered}

各ファセットの背景にトータルの流行曲線を表示するには、**ggplot** に **gghighlight** パッケージの `gghighlight()` を追加します。すべてのファセットにおけるy軸の最大値が、トータルの流行曲線にしていることに注意して下さい。このパッケージの例は \[ggplotの基礎\] ページで多く紹介されています。
...
全データを含むファセットボックスを表示したい場合は、データセット全体を複製し、複製したものを1つのファセットとして扱います。以下にコーディングした自作関数である「helper」関数 `CreateAllFacet()` により行うことが出来ます([このページ](https://stackoverflow.com/questions/18933575/easily-add-an-all-facet-to-facet-wrap-in-ggplot2)で紹介されています)。この関数を実行すると、行の数が2倍になり、`facet` という新しい列が作られます。その中で新たに複製された行は「all」という値を、元の行は `facet` 列の元の値を持つようになります。その後にこの `facet` 列に対してファセットを行います。

以下は helper 関数です。これを実行し、利用出来るようにします。
...
次にこの helper 関数をデータセットの `age_cat` 列に適用します。
...
-   使用するデータは central_data2 (行数が 2 倍になり、新しい列 "facet" が追加)

-   Labeller を使用する場合は、更新する必要があります。

-   オプション：縦に積み重ねたファセットを実現するために：ファセット列を式の行側に移動し、右側を "." (`facet_wrap(facet~.)`) に置き換え、`ncol = 1`とします。保存される png ファイルの幅と高さを調整する必要があるでしょう (\[ggplotの基礎\] の `ggsave()` を参照してください)。
...
流行曲線で示される最新のデータは、多くの場合、暫定的な値であり、報告遅れのために後から症例数が追加される可能性があることを明示した方が良いでしょう。これは、直近の日数分を垂直線や長方形で囲むことで行うことが出来ます。ここでは、2つの方法を紹介します。
...
2)  データを暫定的なステータスでグループ化し、それらの棒グラフを異なる色で表示します。

[***注意:*** 長方形の作成のために `geom_rect()` を試すかも知れませんが、ラインリストの場合は上手くいきません。この関数は、各観測点/行に対して1つの長方形を重ねます! 非常に低いアルファ値 (例えば 0.01) を使用するか、他の方法を用いてください。]{style="color: orange;"}

### `annotate()` の使用 {.unnumbered}

-   `annotate(geom = "rect")` 内で、 `xmin` と `xmax` の引数は日付型である必要があります。

-   これらのデータは週単位の棒グラフに集約されており、最後の棒グラフは最後のデータポイントの次の月曜日まで伸びているので、シェードのエリアは4週間をカバーしているように見えるかもしれないことに注意してください。\

-   `annotate()` の例は[こちら](https://ggplot2.tidyverse.org/reference/annotate.html)です。
...
同じ黒い縦線の追加は以下のコードでも可能ですが、`geom_vline()` を使うと、高さをコントロールする機能が失われます：
...
### 棒グラフの色を変える {.unnumbered}

別の方法として、暫定データに対して棒グラフ自体の色や表示を調整することも考えられます。データ準備段階で新しい列を作り、それを使ってデータをグループ化し、暫定データの `aes(fill = )` を他の棒グラフと異なる色やアルファ値にすることができます。
...
月や年など複数レベルの日付ラベルを、下位のラベルと重複することなく表示したい場合は、以下の方法のいずれかを検討して下さい：

`date_labels` や `labels` の引数で、各ラベルの一部を下の行に配置するために `\n` 使うことができることも可能です。しかし、以下のコードでは、例えば年や月を下の行に、しかも一度だけ表示することができます。注意点は以下の通りです：

-   症例数は、外観上の理由から週単位で集計されています。詳細はこのページの**集計データ**のセクションを参照して下さい。
...
上記の方法は、stackoverflow.comの[こちら](https://stackoverflow.com/questions/44616530/axis-labels-on-two-lines-with-nested-x-variables-year-below-months)と[こちら](https://stackoverflow.com/questions/44616530/axis-labels-on-two-lines-with-nested-x-variables-year-below-months)の投稿を参考にしています。

<!-- ======================================================= -->

## 2軸グラフ

データ可視化コミュニティーの中では、2軸グラフの有効性について激しい議論が交わされていますが、流行曲線やそれに類するグラフで、2軸目に割合を重ねたものを見たいと思う疫学専門家は依然として多くいます。これについては\[ggplotの基礎\]のページで詳しく説明していますが、**cowplot** メソッドを使った一例を以下に示します：

-   2つの異なるプロットを作成し、**cowplot** パッケージで結合します。\
-   2つのプロットは全く同じx軸（範囲を設定する）を持つ必要があり、そうでないとデータとラベルがずれてしまいます。\
-   それぞれ `theme_cowplot()` を使用し、一方はy 軸をプロットの右側に移動します。
...
次に **cowplot** を使って2つのプロットを重ね合わせます。x軸を合わせること、y軸のサイド、`theme_cowplot()` の使用に注意します。
...
注意： **incidence2** を使用する場合、簡単な関数で累積症例数を生成する方法のセクションを参照してください。このページでは、累積症例数を計算し、`ggplot()` でプロットする方法について述べます。

症例のラインリストから始める場合、ベース R の `cumsum()` を使用して、アウトブレイクにおける日ごとの累積症例数の列を新たに作成します。
...
この累積症例数の列は、`geom_line()` を使用して、`date_onset` に対してプロットすることが出来ます。
...
また、上記や \[ggplotの基礎\] ページで説明されている cowplot を使って、2 軸で流行曲線に重ねることも出来ます：

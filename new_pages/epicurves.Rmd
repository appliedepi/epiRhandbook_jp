...
本章では、高度なカスタマイズが可能な **ggplot2** パッケージを使用して流行曲線を作成する方法を示します。
...
流行曲線を描画する **incidence2** パッケージでは、より簡単なコマンドを使用した代替をとっていますが、本章の執筆時点では開発中であり、安定していませんでした。 安定したらこの章に改めて追加します。
...
scale_x_date(
    ...
    # 日付ラベルのフォーマット
    #date_labels = "%a\n%d %b\n%Y")+         # 曜日＋日付と省略月名と年
    label = scales::label_date_short())      # 日付ラベルの自動フォーマット
```

...
レポートを作成する際に、データが最新である日付（「データ日」）などの編集可能なパラメータを設定したい場合があります。これにより、フィルターやダイナミックキャプションを適用する際に、コード内でオブジェクトdata_dateを参照することができます
...
You can optionally specify a value of class Date to `firstdate =` in the `incidence()` コマンドでは日付型（例： `as.Date("2016-05-01")` )の値を `firstdate =` として指定することが出来ます。その場合、データはこの範囲で切り取られ、指定した日を集計単位の最初とすることが出来ます。
...
`plot()` の `col_pal =` 引数に、定義済みの色パレットの名前を指定します。**incidence2** パッケージには、「vibrant 」と 「muted 」という2つの色パレットが付属しています。「vibrant 」では最初の6色が、「muted 」では最初の9色がそれぞれ異なる色となっています。それ以降の色は、他の色の補間となります。これらの定義されたパレットは、[このウェブサイト](https://personal.sron.nl/~pault/#sec:qualitative) で見ることができます。これらのパレットには灰色が含まれていませんが、これは欠損値があった場合に用いられるためです（`na_color =` を指定することでこのデフォルトを変更出来ます）。

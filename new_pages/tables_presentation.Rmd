...
<<<<<<< Updated upstream
このページでは **flextable**パッケージを使って、要約データフレームを見やすい表に変換する方法を紹介します。これらの表は、パワーポイントのスライド、HTML ページ、PDF や Word 文書などに挿入することができます。
=======
このページでは **flextable**パッケージを使って、要約したデータフレームを見やすい表に変換する方法を紹介します。これらの表は、パワーポイントのスライド、HTML ページ、PDF や Word 文書などに挿入することができます。
>>>>>>> Stashed changes

**flextable**を使用する <u>前</u> に、要約表をデータフレームとして作成する必要があります。[記述統計表の作り方]()および[データの縦横変換]()の章に紹介されている方法を使い、表計算、クロス集計などを作成してください作成されたデータフレームを **flextable** に渡して、見やすいように書式設定を行うことができます。


本章では **flextable** を紹介していますが、他にも見やすい表を作成するために利用できる R パッケージはたくさんあります。 **knitr** パッケージとそこに含まれる `kable()`関数を使った例は、[接触者の追跡]()の 章で紹介しています。同様に、**DT** パッケージは [Shiny で作るダッシュボード]() の章で紹介しています。**GT** や **huxtable** のような他のパッケージは、[推奨するパッケージ]() の章で紹介しています。
...
**flextable** をインストールして読み込みます。このハンドブックでは**pacman**の `p_load()` を推しています。p_load() では必要に応じてパッケージをインストール *し* 、使用するために読み込んでくれます。また、R の **ベース** パッケージ（以下、ベース R）の `library()` を使ってパッケージを読み込むこともできます。 R のパッケージについての詳細は [R の基礎]() の章をご覧ください。
...
まずはじめに, エボラ出血熱の伝染病をシミュレーションしたデータクリーニング済みの症例データセットをインポートします。 データを操作しながら行う場合は, <a href='https://github.com/epirhandbook/Epi_R_handbook/raw/master/data/case_linelists/linelist_cleaned.rds' class='download-button'>「クリーニング済み」のラインリストをダウンロードします。</a> (.rds ファイル)。**rio** パッケージの `import()` 関数を使ってデータをインポートします（.xlsx、.csv、.rds などさまざまな種類のファイルを扱うことができます － 詳細は [データのインポート・エクスポート]() の章をご覧ください）。
...
<<<<<<< Updated upstream
**flextable** を使い始める *前* に、テーブルをデータフレームとして <u>作成する</u> 必要があります。 [記述統計表の作り方]()および[データの縦横変換]()の章をご覧ください。その後、データフレームを **flextable** に渡して、色、ヘッダ、フォントなどを設定していきます。
=======
**flextable** を使い始める *前* に、テーブルをデータフレームとして <u>作成する</u> 必要があります。**janitor** や **dplyr** などのパッケージを利用してデータフレームを作成する方法を学ぶには [記述統計表の作り方]()および[データの縦横変換]()の章をご覧ください。表を表示したい内容に行と列を整えることが必要です。その後、データフレームを **flextable** に渡して、色、ヘッダ、フォントなどを設定していきます。
>>>>>>> Stashed changes
...
**flextable** オブジェクトを作成・管理するために、まずデータフレームを `flextable()` 関数に渡します。その結果を `my_table` として保存します。
...
<<<<<<< Updated upstream
このようにして、 `my_table` オブジェクトを、より多くの **flextable**のフォーマット関数に次々とパイプ演算子で渡していくことができます。
=======
このようにした後、 `my_table` オブジェクトをさらに **flextable** に含まれる表をフォーマットするための関数に次々とパイプ演算子で渡していくことができます。
>>>>>>> Stashed changes

このページでは、わかりやすくするために、途中の段階でテーブルを `my_table`として保存し、段階的に **flextable** の機能を追加していきます。最初から最後までの <u>すべて</u> のコードをまとめて見たい場合は、下記の[すべてのコードをまとめる](#tbl_pres_all)セクションをご覧ください。

**flextable** コードの各行の一般的な構文は以下の通りです。:
...
**flextable** フォーマット関数の全リストは [ここ](https://davidgohel.github.io/flextable/reference/index.html) でご覧いただけます。また、 `?flextable`と入力してドキュメントを確認することもできます。
=======
>>>>>>> Stashed changes
...
>>>>>>> Stashed changes
`autofit()` 関数を使うと、各セルに 1 行分のテキストしか入らないように表をうまい具合に引き伸ばすことができます。関数`qflextable()` は、 `flextable()` および `autofit()`を簡潔にした便利なものです。
...
その代わりに、 `width()` 関数で幅を指定することができます。以下の例では、1 列目、2 列目、4 列目から 8 列目にそれぞれ異なる幅を指定しています。
...
この表では、同じサブグループをカバーする列がグループ化されるように、2 つ目のヘッダレイヤを追加したいと思います。そのために、 `top = TRUE`の `add_header_row()` 関数を使用します。各列の新しい名前を `values =`に指定し、 後で統合することがわかっている列には空の値`""` を指定します。
...
罫線や内線などの調整は、さまざまな **flextable** 関数で行うことができます `border_remove()`で既存の罫線をすべて削除することから始めると、簡単な場合が多いです。
...
これらのすべての関数において、実際の罫線のスタイル自体は `border =` で指定する必要があり、 **officer** パッケージの `fp_border()` 関数を使用した別のコマンドで出力しなければなりません。この関数は、線の幅と色を定義するのに役立ちます。以下のように、表コマンドの上で定義することができます。
...
**flextable**`align()` 関数を使って、病院名のある左端の列以外のすべての列を中央揃えにします。
...
関数`colformat_num()`を使用して、比率の列のみを小数点 1 桁まで表示することができます。なお、これはデータ管理の段階で `round()` 関数を用いて行うこともできます。
...
**flextable** 出力の 'live' バージョンを意図したドキュメント形式で表示したい場合は、 `print()` を使用し、 `preview =`に以下のいずれかを指定します。ドキュメントは、指定したソフトウェアプログラムでコンピュータ上に「ポップアップ」して開きますが、保存はされません。引数の preview に "pptx " または "docx " を指定して print 関数を使用することができます。

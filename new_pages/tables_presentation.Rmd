...
table <- linelist %>% 
  ...
  bind_rows(                                           # 前の表とこのミニ表の合計を結合する
    linelist %>% 
      ...
      group_by(outcome) %>%                            # 病院ではなく、転帰のみでグループ化     
      summarise(
        N = n(),                                       # データセット全体の行数     
...
**flextable**を使用する *前* に、要約表をデータフレームとして作成する必要があります。［記述統計表の作り方］および［データの縦横変換］の章に紹介されている方法を使い、表計算、クロス集計などを作成してください作成されたデータフレームを **flextable** に渡して、見やすいように書式設定を行うことができます。
...
# linelist データの最初の 50 行を以下に表示します。
DT::datatable(head(linelist, 50), rownames = FALSE, filter="top", options = list(pageLength = 5, scrollX=T), class = 'white-space: nowrap' )
```

### テーブルの準備 {.unnumbered}

**flextable** を使い始める *前* に、テーブルをデータフレームとして *作成する* 必要があります。 ［記述統計表の作り方］および［データの縦横変換］の章をご覧ください。その後、データフレームを **flextable** に渡して、色、ヘッダ、フォントなどを設定していきます。
...
table <- linelist %>% 
  ...
  ########################
  mutate(hospital = replace_na(hospital, "Total")) %>% 
  pivot_wider(                                         # 縦から横へのピボット変換
    values_from = c(ct_value, N),                       # CT値とカウント列からの新規の値
    names_from = outcome) %>%                           # outcomeから新規の列名
  mutate(                                              # 新しい列の追加
    N_Known = N_Death + N_Recover,                               # 転帰がわかっている症例数
    Pct_Death = scales::percent(N_Death / N_Known, 0.1),         # 死亡症例のパーセント（小数点1桁）
    Pct_Recover = scales::percent(N_Recover / N_Known, 0.1)) %>% #回復症例のパーセント（小数点1桁）
...
このページでは、わかりやすくするために、途中の段階でテーブルを `my_table`として保存し、段階的に **flextable** の機能を追加していきます。最初から最後までの *すべて* のコードをまとめて見たい場合は、下記の[すべてのコードをまとめる](#tbl_pres_all)セクションをご覧ください。
...
しかし、セル内の値が非常に長く、表がページに収まらない場合など、必ずしも適切ではない場合があります。 その代わりに、 `width()` 関数で幅を指定することができます。以下の例では、1 列目、2 列目、4 列目から 8 列目にそれぞれ異なる幅を指定しています。
...
my_table <-  my_table %>%  
  ...
  bold(i = 7, bold = TRUE, part = "body")           # Total行の太字を調整する（本文7行目）
...
table <- linelist %>% 
  ...
  add_header_row(
    ...
    set_header_labels(         # 元のヘッダ行の列名を変更
      ...
      Pct_Death = "% of cases",
      ct_value_Death = "Median CT values")  %>% 
  merge_at(i = 1, j = 3:5, part = "header") %>% # 3 列目から 5 列目までを新しいヘッダ行に水平に結合
  merge_at(i = 1, j = 6:8, part = "header") %>%  
  border_remove() %>%  
  theme_booktabs() %>% 

...
データを管理する上で、[ピボットする]{.ul}とは次の2つのプロセスのどちらかを指しています。

1.  [ピボットテーブル]{.ul}（より大きなテーブルデータを要約した統計表）を作成すること。\
2.  表を**ロング**形式から**ワイド**形式へ、またはその逆へ変換すること。

**このページでは、後者の定義に焦点をあてます。**前者はデータ解析の重要なステップであり、[データのグループ化](https://epirhandbook.com/en/grouping-data.html#grouping-data)および[記述統計表の作り方](https://epirhandbook.com/en/descriptive-tables.html#descriptive-tables)ページで別に取り上げています。\
このページでは、データの形式について説明します。各変数がそれぞれ列を持ち、各観測値がそれぞれ行を持ち、各値がそれぞれセルを持つという「整頓されたデータ」の考え方を知っておくと便利です。このトピックについて詳しくは、[R for Data Scienceの章](https://r4ds.had.co.nz/tidy-data.html)を参照してください。

...
[[**注意**]{.ul}**：**以下の例では、`linelist` データフレームを `select()` で変更して表示していますが、これは例としてデータを示すためであり、変更されたデータは保存はされていないことに注意してください。変更された列名は、データフレームを `names()` にパイプすることで表示されます。]{style="color: orange;"}
...
```{r}
# ラインリストのデータセットは select() に渡され、name() で列名を表示する
linelist %>% 
...
新しい列を追加したり、既存の列を修正したりする場合には、**dplyr** 系関数の `mutate()` を使用することをおすすめします。
...
以下は、`mutate()` を使用して新しい列を作成する最も基本的なコマンドはです。ここでは、すべての行の値が 10 である新しい列 `new_col` を作成します。
...
また、他の列の値を参照して、計算を行うこともできます。以下では、`bmi` という新しい列が作成され、各症例の Body Mass Index（BMI）を格納しています。これは、既存の列である `ht_cm` と `wt_kg` を使用して、BMI = kg/m\^2 の式で計算されたものです。
...
新しい列を確認します。デモンストレーションのために、新しい列とそれを作成するために使用された列のみが表示されています。
...
[[**ヒント**]{.ul}**：**`mutate()` の変化形として、`transmute()` という関数があります。この関数は、`mutate()` と同じように新しい列を追加しますが、括弧の中に書かれていない他のすべての列を削除または除外します。]{style="color: darkgreen;"}
...
日付、数字、理論値（TRUE/FALSE）を含む列は、正しく分類された時のみ期待通りの動きをします。例えば、文字型の "2" と数字型の 2 は異なることに注意してください。
...
では、具体的に見ていきましょう。現在、`age` 列のデータ型は 文字列型（character）ですが、定量的な分析を行うためには、これらの数字が数値として認識される必要があります！

```{r}
class(linelist$age)
```

`date_onset` 列のデータ型も文字列型です！分析を行うためは、これらの日付が日付として認識される必要があります。

```{r}
class(linelist$date_onset)
```

これを解決するために、`mutate()` の機能を使って、変換で列を再定義します。列の定義に変更はありませんが、異なるデータ型に変換します。ここでは、基本的な例として、`age` 列が数値型（numeric）に変換します。
...
グループ化されたデータフレームでの `mutate ()` の使用については、こちらの [tidyverse mutate に関するドキュメント](https://dplyr.tidyverse.org/reference/mutate.html) で詳しく説明されていますので、必要な方はご覧ください。

### 複数列の一斉変換 {#clean_across .unnumbered}

コードを簡潔にするために、同じ変換を行うコマンドを一度で複数の列に適用したい場合があります。**dplyr** パッケージ（ **tidyverse** パッケージにも含まれています）の `across()` を使用すると、データの変換を行うコマンドを一度に複数の列に適用することができます。 `across()` はすべての **dplyr** 系関数で使用することができますが、一般的には `select()`、`mutate()`、`filter()`、`summarise()` 内でよく使用されます。`across()` をどのように `summarise()` で使用するかについて知りたい方は、[記述統計表の作り方](#tables-descriptive) の章をご覧ください。
...
また、別の例として、現在 POSIXct 型（未加工の日付時分秒（date-time）を示すデータ型）の列を日付型（date）に変換する例を示します。まず、`is.POSIXct()` の評価値が `TRUE` である列を `where()` で検索し、次に、これらの列に `as.Date()` を適用して、通常の 日付型（date）に変換します。
...
`across()` に関数を与える方法の詳細については、`?across` をコンソールで実行して表示されるドキュメントをお読みください。要約すると、列に対して実行する関数を指定する方法はいくつかあり、独自の関数を定義することもできます。
...
`coalesce()` は、**dplyr** パッケージに含まれており、指定された値のうち、欠損値ではない最初の値を見つける関数です。この関数は、指定された順序で、取得された欠損地ではない最初の値で欠損値を埋めます。

ここでは、データフレーム以外の例を示します。2 つのベクトルがあるとします。1 つは患者の発見された村、もう 1 つは患者の居住地の村です。`coalesce()` を使用し、各インデックスの最初の非欠損値を見つけることができます。
...
値を手動で変更するためには、`mutate()` の中の `recode()` を使用します。

例えば、データの中に意味のない日付（例："2014-14-15"）があるとします。未加工の元データ上でマニュアルで日付を修正することもできますし、`mutate()` と `recode()` を使用して前処理パイプラインに変更を書き込むこともできます。後者の方がより透明性と再現性が高く、行われた分析を理解しようとする第三者や同じように分析しようとする人にとって、分析を再現しやすくなります。
...
上の例の `mutate()` の行は、「`date_onset` 列の間違っている既存の値が新しい値に変更されるように変換し、`date_onset` 列に再定義する」と読むことができます。`recode()` のパターン（既存の値 = 新しい値）は、ほとんどの R の関数のパターン（新しい値 = 既存の値）とは逆であることに注意してください。R の開発コミュニティでは、この相違の修正に取り組んでいます。

**以下に、別の例として、1 つの列において複数の値を再定義する例を紹介します。**

`linelist` の `hospital` 列には、いくつかスペルが間違った値があることに加え、多くの欠損値があるため、`hospital` 列の値をクリーニングする必要があります。
...
上のコードを実行すると、 `hospital` 列内で間違ったスペルの値が修正され、正しいスペルの値のグループに統合されていることがわかります。

```{r}
table(linelist$hospital, useNA = "always")
```

[[**ヒント**]{.ul}：等号（=）の前後には、スペースをいくつか入れても問題ありません。コードを読みやすくするために、すべての行またはほとんどの行で等号（=）の場所を揃えてください。また、ハッシュタグでコメント行を追加し、どちらが古い値でどちらが新しい値なのかを、第三者に明確にわかるようにすることも大事なポイントです。]{style="color: darkgreen;"}

[[**ヒント**]{.ul}：データセットの中には、空白文字の値が存在することがあります（これは、R の欠損値である NA として認識されません）。このような値を参照するためには、2 つの引用符をスペースを空けずに（""）使用します。]{style="color: darkgreen;"}
...
`replace()` は **base** R の関数で、論理条件を使って変更する行を指定します。一般的な構文は次のとおりです：

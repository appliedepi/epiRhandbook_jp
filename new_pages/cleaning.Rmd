...
　行内で計算を行いたい場合は、**dplyr** の `rowwise()` を利用することができます。例えば、このコードでは `rowwise()` を適用し、行リストの各行について、値が"はい"である指定された症状の列の数を合計する新しい列を作成しています。`rowwise()` は本質的に特別な種類の `group_by()` であるため、終わったら `ungroup()` を使うのがベストです（ [Grouping data](https://epirhandbook.com/en/grouping-data.html#grouping-data) ページを参照）。

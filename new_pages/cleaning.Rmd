...
**skimr** パッケージの `skim()` を使うと、データフレーム全体の概要を把握することができます（詳しくは、[記述統計表の作り方](#tables-descriptive) を参照ください）。 [Descriptive tables](https://epirhandbook.com/descriptive-tables.html#descriptive-tables) のページを参照）。列は、文字値や数値など、データ型別にまとめられています。

注釈："POSIXct" は未加工の日付データ型の一つです（詳細は、[Working with dates](https://epirhandbook.com/working-with-dates.html#working-with-dates-1) を参照してください。

...
本章では、記述的分析のためにデータをグループ化および集約する方法について説明します。 **tidyverse** ファミリーのパッケージが一般的で機能が使いやすいため、これを使用します。
...
デフォルトでは、すでにグループ化されているデータに対して `group_by()` を実行すると、古いグループが削除され、新しく作成されたグループが適用されます。既に作成されたグループに新しいグループを追加したい場合は、引数に `.add = TRUE` を含めてください。
...
**dplyr** 関数 `summarise()` （または `summarize()`）はデータフレームを受け取り、あなたの定義した要約統計量を含む、新しい要約データフレームに変換します。グループ化されていないデータフレームでは、サマリー統計量はデータフレーム内のすべての行から計算されます。`summarise()` がグループ化されたデータに適用されると、各グループの要約統計量が生成されます。
...
`count()` と `summarise()` とは対照的に、`add_count()` を使用すると、[データフレーム内の他のすべての列を保持しつつ]{.ul}、グループごとの行数を含む新しい列 `n` を[追加]{.ul}できます。
...
`tally()` か `count()` を使用した後、合計行または列の[合計]{.ul}を簡単に追加するには、[記述統計表の作り方](#Descriptive%20tables) の章の **janitor** セクションを参照してください。**janitor** パッケージは、`adorn_totals()` や `adorn_percentages()` のような、合計を追加してパーセンテージを表示する関数を提供します。以下は簡単な例です。
...
こちらは `complete()` を使用[せずに]{.ul}、症例を日数でグループ化する例です。以下の例では、最初の行で症例のない日をスキップすることに注意してください。
...
2)  [各病院グループ内で]{.ul} `date_hospitalisation` 列を最新の日付から古い日付へ降順で並べ替える\
